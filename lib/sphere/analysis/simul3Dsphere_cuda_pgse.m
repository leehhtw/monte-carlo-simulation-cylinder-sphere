classdef simul3Dsphere_cuda_pgse < handle
    
    properties (GetAccess = public, SetAccess = public)
        TD;
        dtime; Tstep; NPar; Din; Dex; kappa; pinit;
%         dt_lls; adSig; rdSig; mdSig; akSig; rkSig; mkSig;
        dx2t; dx4t; sig;
        Del; del; bval; bvec;
%         dx2tSig; dx4tSig;
%         dx2tShl; dx4tShl;
    ...
    methods (Access = public)
        function this = simul3Dsphere_cuda_pgse(root)
            if ~isempty(root)
                this.readJob(root);
            end
        end
        ...
        function this = readJob(this,root)
            ...
            btab = load(fullfile(root,'gradient_time.txt'));
            btab = reshape(btab,6,[]).';
            this.Del = btab(:,1);
            this.del = btab(:,2);
            grad = btab(:,3);
            this.bval = grad.^2.*this.del.^2.*(this.Del-this.del/3);
            this.bvec = btab(:,4:6);
            
%             btab = load(fullfile(root,'btable.txt'));
%             btab = reshape(btab,4,[]).';
%             this.bval = btab(:,1);
%             this.bvec = btab(:,2:end);
%             sigIm = load(fullfile(root,'sigIm.txt'));
%             this.sig = sqrt(sigRe.^2 + sigIm.^2)/this.NPar;
%             this.sig = abs(sigRe)/this.NPar;
        end
