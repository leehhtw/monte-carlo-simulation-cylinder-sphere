...
//  -- 4/11/2020: implement signals
//  -- 4/23/2020: replace pow with pow2, fix the bug in translateFlag
//
...
__device__ void pixPosition ( const double x_in[], const unsigned int &NPix, int xPix[] ) {
    double x[3]={0}; x[0]=x_in[0]; x[1]=x_in[1]; x[2]=x_in[2];
...
__device__ void translateXc ( const double x[], double xc[] ) {
    ...
    double ti=0, tj=0, tk=0;
    double d2 = pow2(x[0]-xc[0])+pow2(x[1]-xc[1])+pow2(x[2]-xc[2]), d2Tmp=0;
    int ii[2]={0}, jj[2]={0}, kk[2]={0};
    ii[1]=2*(xc[0]<0.5)-1;
    jj[1]=2*(xc[1]<0.5)-1;
    kk[1]=2*(xc[2]<0.5)-1;
    for (int i=0; i<2; i++) {
        for (int j=0; j<2; j++) {
            for (int k=0; k<2; k++) {
                ...
                d2Tmp=pow2(x[0]-xc[0]-ii[i])+pow2(x[1]-xc[1]-jj[j])+pow2(x[2]-xc[2]-kk[k]);
                if (d2Tmp<d2) {
                    d2=d2Tmp;
                    ti=ii[i];
                    tj=jj[j];
...
__device__ bool inICS ( const double x[], const double xc_in[], const double &rc, const bool &translateFlag ) {
    double xc[3]={0}; xc[0]=xc_in[0]; xc[1]=xc_in[1]; xc[2]=xc_in[2];
    ...
    return ( ( pow2(x[0]-xc[0])+pow2(x[1]-xc[1])+pow2(x[2]-xc[2]) ) <= rc*rc );
}

__device__ bool stepE2I (const double xi[], const double xt[], const double xc_in[], const double &rc, double t[], const double &dx, const bool &translateFlag) {
    double xc[3]={0}; xc[0]=xc_in[0]; xc[1]=xc_in[1]; xc[2]=xc_in[2];
    ...
    t[0]=-( (xi[0]-xc[0])*(xt[0]-xi[0])+(xi[1]-xc[1])*(xt[1]-xi[1])+(xi[2]-xc[2])*(xt[2]-xi[2]) )/dx/dx;
    
    // If xt is in the cell, segment overlaps the circle.
    if ( ( pow2(xt[0]-xc[0])+pow2(xt[1]-xc[1])+pow2(xt[2]-xc[2]) ) <= rc*rc ) {
        return true;
    } else {
        ...
        xl[2]=xi[2]+(xt[2]-xi[2])*t[0];
        double d2=pow2(xl[0]-xc[0])+pow2(xl[1]-xc[1])+pow2(xl[2]-xc[2]);
        ...
        if (d2>rc*rc) {
            ...
            return ( ( (xi[0]-xl[0])*(xt[0]-xl[0])+(xi[1]-xl[1])*(xt[1]-xl[1])+(xi[2]-xl[2])*(xt[2]-xl[2]) ) <= 0 );
        }
    }
}

__device__ void elasticECS (const double x[], const double v[], const double &dx, const double xc_in[], const double &rc, const bool &translateFlag, double xt[]) {
    double xc[3]={0}; xc[0]=xc_in[0]; xc[1]=xc_in[1]; xc[2]=xc_in[2];
    ...
    b=2*(x[0]-xc[0])*v[0] + 2*(x[1]-xc[1])*v[1] + 2*(x[2]-xc[2])*v[2];
    c=pow2(x[0]-xc[0]) + pow2(x[1]-xc[1]) + pow2(x[2]-xc[2]) - rc*rc;
    ...
    if (discri<=0) {                    // Does not encounter the cell membrane
        ...
        if ( (t>=dx) | (t<0) ) {        // Does encounter the cell membrane
            ...
            t1=sqrt( pow2(xc[0]-xm[0])+pow2(xc[1]-xm[1])+pow2(xc[2]-xm[2]) );
            n[0]=(xm[0]-xc[0])/t1;
            n[1]=(xm[1]-xc[1])/t1;
            n[2]=(xm[2]-xc[2])/t1;
            
            // v' = v - 2*dot(v,n)*n
            t1=v[0]*n[0]+v[1]*n[1]+v[2]*n[2];
            n[0]=v[0]-2*t1*n[0];
            n[1]=v[1]-2*t1*n[1];
            n[2]=v[2]-2*t1*n[2];
...
__device__ void permeateE2I (const double x[], const double v[], const double &dxEX, const double xc_in[], const double &rc, const double &dxIN, const bool &translateFlag, double xt[]) {
    double xc[3]={0}; xc[0]=xc_in[0]; xc[1]=xc_in[1]; xc[2]=xc_in[2];
    ...
    if (discri<=0) {                    // Does not encounter the cell membrane
        ...
        if ( (t>=dxEX) | (t<0) ) {      // Does encounter the cell membrane
            ...
            t1=fabs(v[0]*n[0]+v[1]*n[1]+v[2]*n[2]);
            xt[0]=xm[0]+n[0]*(1-t/dxEX)*dxIN*t1;
            xt[1]=xm[1]+n[1]*(1-t/dxEX)*dxIN*t1;
            xt[2]=xm[2]+n[2]*(1-t/dxEX)*dxIN*t1;
        }
    }
}

__device__ void elasticICS (const double x[], const double v[], const double &dx, const double xc_in[], const double &rc, const bool &translateFlag, double xt[]) {
    double xc[3]={0}; xc[0]=xc_in[0]; xc[1]=xc_in[1]; xc[2]=xc_in[2];
...
__device__ void permeateI2E (const double x[], const double v[], const double &dxIN, const double xc_in[], const double &rc, const double &dxEX, const bool &translateFlag, double xt[]) {
    double xc[3]={0}; xc[0]=xc_in[0]; xc[1]=xc_in[1]; xc[2]=xc_in[2];
    ...
    if (discri<=0) {                     // Walker is right on the surface and diffuses tangent to the surface
        ...
        if ( t>=dxIN ) {                // Does not encounter the cell membrane
            ...
            xt[2]=xm[2]+n[2]*(1-t/dxIN)*dxEX*t1;
        }
    }
}
...
__global__ void propagate(curandStatePhilox4_32_10_t *state, double *dx2, double *dx4, double *NParICS, double *NParBin, double *sig, const int TN, const int NPar, const int Nbvec, const double res, const double stepIN, const double stepEX, const double probI2E, const double probE2I, const unsigned int NPix, const unsigned int Nmax, const int initFlag, const double *xCir, const double *yCir, const double *zCir, const double *rCir, const bool *translateFlag, const unsigned int *APix, const double *btab, const double *TD){
    int idx = threadIdx.x + blockDim.x * blockIdx.x;
    ...
    for (int k=idx; k<NPar; k+=stride){
        ...
        int xParG[3]={0};                                   // Particle position on a grid
        
        bool instruction1=false, instruction2=false;
        unsigned int a=0, aTmp=0;                           // Element of APix matrix
        unsigned int a1=0, a2=0;                            // Label of cells close to the particle
        unsigned int acell[4]={0};                          // Cell label
        bool instruction[4]={false};
        ...
        int xtG[3]={0}, xTmpG[3]={0};                       // Position on grid after diffusion
        double vrand=0;                                     // Random number
        double cos_theta=0, sin_theta=0;
        int tidx=0, bidx=0;
        
        double vp[3]={0};                                   // Nomalized diffusion velocity
        unsigned int acell_hit=0;                                    // Label of the cell encountered by the walker
        ...
        while (1){
            ...
            if ( initFlag==1 ) { // 1. Initial positon: ICS
                ...
                a=APix[ NPix*NPix*xParG[0]+NPix*xParG[1]+xParG[2] ];
                a1=a%Nmax; a2=a/Nmax;
                
                // If the walker is in ICS, take the initial position
                instruction1=false; instruction2=false;
                if ( a1 ){
                    xCirTmp[0]=xCir[a1-1]; xCirTmp[1]=yCir[a1-1]; xCirTmp[2]=zCir[a1-1];
                    instruction1=inICS(xPar,xCirTmp,rCir[a1-1],translateFlag[a1-1]);
                }
                
                if ( a2 ){
                    xCirTmp[0]=xCir[a2-1]; xCirTmp[1]=yCir[a2-1]; xCirTmp[2]=zCir[a2-1];
                    instruction2=inICS(xPar,xCirTmp,rCir[a2-1],translateFlag[a2-1]);
                }
        ...
        for (int i=0; i<TN; i++){
            ...
            a=APix[ NPix*NPix*xtG[0]+NPix*xtG[1]+xtG[2] ];
            a1=a%Nmax, a2=a/Nmax;
            
            // Check if the particle is in ICS
            instruction1=false; instruction2=false;
            if ( a1 ) {
                xCirTmp[0]=xCir[a1-1]; xCirTmp[1]=yCir[a1-1]; xCirTmp[2]=zCir[a1-1];
                instruction1=inICS(xt,xCirTmp,rCir[a1-1],translateFlag[a1-1]);
            }
            
            if ( a2 ) {
                xCirTmp[0]=xCir[a2-1]; xCirTmp[1]=yCir[a2-1]; xCirTmp[2]=zCir[a2-1];
                instruction2=inICS(xt,xCirTmp,rCir[a2-1],translateFlag[a2-1]);
            }
            ...
            while (iterateFlag & (ite<nite)) {
                if ( (instruction1==false) & (instruction2==false) ) {
                    ...
                    xTmp[2]=xt[2]+stepEX*vp[2];
                    
                    pixPosition(xTmp,NPix,xTmpG);
                    aTmp=APix[ NPix*NPix*xTmpG[0]+NPix*xTmpG[1]+xTmpG[2] ];
                    ...
                    for (int j=0; j<4; j++) {
                        ...
                        if ( acell[j] ) {
                            ...
                            xCirTmp[2]=zCir[acell[j]-1];
                            instruction[j]=stepE2I(xt, xTmp, xCirTmp, rCir[acell[j]-1], tcellTmp, stepEX, translateFlag[acell[j]-1]);
                            tcell[j]=tcellTmp[0];
                        }
                    }
                    if ( (instruction[0]==false) & (instruction[1]==false) & (instruction[2]==false) & (instruction[3]==false) ) {
                        ...
                        xCirTmp[2]=zCir[acell_hit-1];
                        
                        vrand=curand_uniform_double(&localstate);
                        if (vrand<probE2I) {
                            // Case 1.2.1 Permeation from ECS to ICS
                            permeateE2I(xt, vp, stepEX, xCirTmp, rCir[acell_hit-1], stepIN, translateFlag[acell_hit-1],xt);
                            iterateFlag=false; ICSFlag=true;
                        }
                        else {
                            // Case 1.2.2 Elastic collision in ECS
                            elasticECS(xt, vp, stepEX, xCirTmp, rCir[acell_hit-1], translateFlag[acell_hit-1],xCollision);
                            ...
                            if ( acell[2] ) {
                                ...
                                xCirTmp[2]=zCir[acell[2]-1];
                                if ( inICS(xCollision, xCirTmp, rCir[acell[2]-1], translateFlag[acell[2]-1]) ) {
                                    xt[0]=xTmp[0]; xt[1]=xTmp[1]; xt[2]=xTmp[2];
                            ...
                            if ( acell[3] ) {
                                ...
                                xCirTmp[2]=zCir[acell[3]-1];
                                if ( inICS(xCollision, xCirTmp, rCir[acell[3]-1], translateFlag[acell[3]-1]) ) {
                                    xt[0]=xTmp[0]; xt[1]=xTmp[1]; xt[2]=xTmp[2];
                                    iterateFlag=true; ite++;
                                }
                            }
                    ...
                    xTmp[2]=xt[2]+stepIN*vp[2];
                    
                    // Check if the segment(xt,xTmp) overlaps with the cell membrane
                    xCirTmp[0]=xCir[acell[0]-1];
                    xCirTmp[1]=yCir[acell[0]-1];
                    xCirTmp[2]=zCir[acell[0]-1];
                    instruction[0]=inICS(xTmp,xCirTmp,rCir[acell[0]-1],translateFlag[acell[0]-1]);
                    ...
                    if ( instruction[0] ) {
                        ...
                        if (vrand<probI2E) {
                            ...
                            permeateI2E(xt, vp, stepIN, xCirTmp, rCir[acell_hit-1], stepEX, translateFlag[acell_hit-1],xCollision);
                            ...
                            // Case 2.2.1.2 Cancel this step and choose another direction if bouncing twice
                            pixPosition(xCollision, NPix, xTmpG);
                            aTmp=APix[ NPix*NPix*xTmpG[0]+NPix*xTmpG[1]+xTmpG[2] ];
                            acell[2]=aTmp%Nmax, acell[3]=aTmp/Nmax;
                            ...
                            elasticICS(xt, vp, stepIN, xCirTmp, rCir[acell_hit-1], translateFlag[acell_hit-1],xCollision);
                            
                            if ( inICS(xCollision, xCirTmp, rCir[acell_hit-1], translateFlag[acell_hit-1]) ) {
            ...
            if ( (i%Tstep) ==0 ) { // Save moment tensor for dx^2 and dx^4, and signal for the b-table
                ...
                if ( initFlag==4 ) {
                    bidx=floor( sqrt( pow2(xt[0]-0.5) + pow2(xt[1]-0.5) + pow2(xt[2]-0.5) )*nbin*2 );
                    if (bidx<nbin) {
...
int main(int argc, char *argv[]) {
    ...
    double Din=0;               // Diffusion coefficient inside the sphere in um^2/ms
    double Dex=0;               // Diffusion coefficient outside the sphere in um^2/ms
    double kappa=0;             // Permeability of a lipid bi-layer in um/ms
    int initFlag=1;             // Initial position: 1=ICS, 2=ECS, 3=ICS+ECS, 4=center
    ...
    double stepIN=sqrt(6.0*dt*Din);     // Step size in ICS in µm
    double stepEX=sqrt(6.0*dt*Dex);     // Step size in ECS in µm
    ...
    thrust::host_vector<unsigned int> APix(NPix*NPix*NPix);
    ifstream myfile3 ("phantom_APix.txt", ios::in);
    for (i=0; i<NPix*NPix*NPix; i++){
        APix[i]=0;
        myfile3>>APix[i];
    }
    myfile3.close();
    ...
    // Circle center of y-coordinate
    thrust::host_vector<double> zCir(NAx);
    ifstream myfile6z ("phantom_zCir.txt", ios::in);
    for (i=0; i<NAx; i++){
        myfile6z>>zCir[i];
    }
    myfile6z.close();
    ...
    for (i=0; i<timepoints; i++){
        TD[i]=static_cast<double>(i*(TN/timepoints)+1)*dt;
    }

    //********** Initialize Particle Positions in IAS *********
    const double probE2I=2.0/3.0*stepEX*kappa/Dex;          // Probability constant from ECS to ICS
    const double probI2E=2.0/3.0*stepIN*kappa/Din;          // Probability constant from IAS to ECS
...
//        translateFlag[i]=true;
        if ( (xCir[i]+rCir[i]+2*Lpix>=1) || (xCir[i]-rCir[i]-2*Lpix<=0) || (yCir[i]+rCir[i]+2*Lpix>=1) || (yCir[i]-rCir[i]-2*Lpix<=0) || (zCir[i]+rCir[i]+2*Lpix>=1) || (zCir[i]-rCir[i]-2*Lpix<=0) ) {
            translateFlag[i]=true;
        } else {
    ...
    thrust::device_vector<double> d_zCir=zCir;
    ...
    begin=clock();
    propagate<<<numBlocks, blockSize>>>(devState.data().get(), d_dx2.data().get(), d_dx4.data().get(), d_NParICS.data().get(), d_NParBin.data().get(), d_sig.data().get(), TN, NPar, Nbvec, res, stepIN, stepEX, probI2E, probE2I, NPix, Nmax, initFlag, d_xCir.data().get(), d_yCir.data().get(), d_zCir.data().get(), d_rCir.data().get(), d_translateFlag.data().get(), d_APix.data().get(), d_btab.data().get(), d_TD.data().get());
    ...
    if (initFlag==4) {
        ...
        for (i=0; i<timepoints; i++) {
            for (j=0; j<nbin; j++) {
                if (j==nbin-1){
                    fNParBinout<<NParBin[i*nbin+j]/(4*Pi*dr*dr*dr*(j*j+j+1.0/3.0))<<endl;
                } else {
                    fNParBinout<<NParBin[i*nbin+j]/(4*Pi*dr*dr*dr*(j*j+j+1.0/3.0))<<"\t\t";
                }
            }
        }
        fNParBinout.close();
    ...
    paraout<<Din<<endl<<Dex<<endl<<kappa<<endl<<initFlag<<endl<<res<<endl;
    paraout.close();
    
    ofstream TDout("diff_time.txt");
    for (i=0; i<timepoints; i++){
        TDout<<static_cast<double>(i*(TN/timepoints)+1)*dt<<endl;
    }
    TDout.close();
}
